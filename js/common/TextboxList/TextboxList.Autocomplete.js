...
TextboxList.Autocomplete = new Class({
	...
	initialize: function(textboxlist, options){
		...
		if (!!(this.options.placeholder || this.options.placeholder === 0) || this.options.queryServer) 
		...
		if (this.options.queryRemote){
			if (this.searchValues[search] && false /* TODO */){
				...
				this.currentRequest = new Request.JSON({url: this.options.remote.url, data: data, onRequest: function(){
					...
					//that.searchValues[search] = data; TODO
					that.values = data;
	...
	addResult: function(r, search){
		var element = new Element('li', {'class': this.prefix + '-result', 'html': Array.pick([r[3], r[1]])}).store('textboxlist:auto:value', r);
	...
	addCurrent: function(){
		...
		if (b){
			...
			b.inject(document.id(this.currentInput), 'before');
		}
		this.blur();
		return this;
	},
...
TextboxList.Autocomplete.Methods = {
    ...
    nofilter: {
        ...
        highlight: function(element, search, insensitive, klass){
            var vals = search.escapeRegExp().split(' ');
            var html = element.get('html');
            vals.each(function(v){
                if(v != ''){
                    var regex = new RegExp('(<[^>]*>)|('+ v.escapeRegExp() +')', insensitive ? 'ig' : 'g');
                    html = html.replace(regex, function(a,b,c){
                        return (a.charAt(0) == '<') ? a : '<strong class="'+ klass +'">' + c + '</strong>';
                    });
                }
            });
