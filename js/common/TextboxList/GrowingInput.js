...
GrowingInput = new Class({
	...
	initialize: function(element, options){
		...
		this.element = document.id(element).store('growing', this).set('autocomplete', 'off');		                                                            		                                                           		
	...
	resize: function(){
		...
		} else if(!!(this.options.max || this.options.max === 0) && this.value.length > this.options.max){
			if(!!(this.options.lastvalue || this.options.lastvalue === 0) && (this.lastvalue.length >= this.options.max)) return;
