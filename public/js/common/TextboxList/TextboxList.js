  ...
  initialize: function(element, options){
    ...
    this.container = new Element('div', {'class': this.options.prefix}).inject(element, 'after');
		this.container.addEvent('click', function(e){ 
			if ((e.target == this.list || e.target == this.container) && (!this.focused || document.id(this.current) != this.list.getLast())) this.focusLast(); 			
		}.bind(this));
	...
	create: function(klass, value, options){
		if (klass == 'box'){
			...
			if ($chk(this.options.max) && this.list.getChildren('.' + this.options.prefix + '-bit-box').length > this.options.max) return false;
	...
	onAdd: function(bit){
		...
		if (bit.is('box')){
            ...
            var next = this.getBit(document.id(bit).getNext());
            if(next && next.is('editable') && $chk(this.options.max) && this.list.getChildren('.' + this.options.prefix + '-bit-box').length + 1 > this.options.max){
                  next.remove();
            }
		}
	},
	onRemove: function(bit){
		if (!this.focused) {
            if($chk(this.options.max) && this.list.getChildren('.' + this.options.prefix + '-bit-box').length == this.options.max){
                if (!bit.is('editable')) this.create('editable', null, {tabIndex: this.original.tabIndex}).inject(this.list);
            }
            return;
        }
	...
	focusRelative: function(dir, to){
		var b = this.getBit(document.id(Array.pick([to, this.current]))['get' + dir.capitalize()]());
		if (b) b.focus();
		return this; 
	},
...
TextboxListBit.Editable = new Class({
  ...
  initialize: function(value, textboxlist, options){
    ...
    this.element = new Element('input', {type: 'text', 'class': this.typeprefix + '-input', autocomplete: 'off', value: this.value ? this.value[1] : ''}).inject(this.bit);		
		if (!!(this.options.tabIndex || this.options.tabIndex === 0)) this.element.tabIndex = this.options.tabIndex;
